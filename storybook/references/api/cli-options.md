...
| `-h`, `--host [string]`         | Host to run Storybook. Setting this to `0.0.0.0` binds to all network interfaces. <br/>Extend it with [`core.allowedHosts`](https://storybook.js.org/docs/api/main-config/main-config-core.md#allowedhosts) to restrict which hosts can access your Storybook instance.<br />`storybook dev -h my-host.com` |
...
| `--skip-automigrations`          | Skips running automigrations entirely, only updating package versions and installing them.<br />`storybook upgrade --skip-automigrations`                                                                                                                                     |
| `-y`, `--yes`                    | Skips interactive prompts and automatically upgrades Storybook to the latest version. Migrations that require a deliberate opt-in, such as framework migrations and experimental feature flags, are not run; use `--features` to include them.<br />`storybook upgrade --yes` |
| `--features <list>`              | Preselects the automigrations that enable the listed experimental feature flags. Other detected automigrations are still offered as usual. Cannot be combined with `--skip-automigrations`.<br />`storybook upgrade --features experimentalReview,experimentalDocgenServer`   |
