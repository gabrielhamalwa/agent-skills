...
The MCP server itself works in every Storybook project.
The [development](#development) and [testing](#testing) toolsets have no framework requirements at all.

The [docs](#docs) toolset is the exception, because it reads the [components manifest](https://storybook.js.org/docs/ai/manifests.md), which not every framework generates yet:

| Framework                                                                                                                  | Features to enable                                  | Component API in `docs-show` |
| -------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ---------------------------- |
| Every React framework (`react-vite`, `react-webpack5`, `nextjs`, `nextjs-vite`, `tanstack-react`, `react-native-web-vite`) | `componentsManifest`                                | Yes                          |
| `@storybook/angular-vite`                                                                                                  | `componentsManifest`                                | Yes                          |
| `@storybook/vue3-vite`                                                                                                     | `componentsManifest` and `experimentalDocgenServer` | Yes                          |
| `@storybook/angular` (Webpack)                                                                                             | No manifest support                                 | No                           |
| Every other framework                                                                                                      | No manifest support                                 | No                           |

`@storybook/angular-vite` needs only the one feature because it enables [`experimentalDocgenServer`](https://storybook.js.org/docs/api/main-config/main-config-features.md#experimentaldocgenserver) itself.
On `@storybook/vue3-vite` that feature is not on by default, so you set both.
Vue setups other than `@storybook/vue3-vite`, including `@nuxtjs/storybook`, generate no manifest.

If your framework generates no components manifest, the docs toolset is unavailable and everything else on this page still applies.
...
### 2. Enable the components manifest

The [docs toolset](#docs) reads the [components manifest](https://storybook.js.org/docs/ai/manifests.md), which is off by default.

Enable it in your `.storybook/main.js|ts`:
...
Your framework does not generate a components manifest yet, so this step does not apply and the [docs toolset](#docs) reports as unavailable.
Skip ahead to the next step and use the [development](#development) and [testing](#testing) toolsets.
...
### 4. Adjust your agent instructions
...
Finally, test your agent's access to the MCP server. First, make sure your Storybook is running, then run a prompt like "List all documented components". You should see a call to the [`docs-list`](#docs-list) tool with a response listing components from your Storybook.
...
Maps component source files to the stories that render them via Storybook's live reverse dependency graph.

#### `review-create`

Pushes a curated review of current changes and returns the review-page URL (requires the review feature to be enabled).
...
Returns the full story and associated documentation for a specific story. The agent calls this tool when `docs-show` doesn't provide enough information for the agent to use a component effectively.
...
We built Storybook's capabilities as an MCP server, which is a standard protocol for AI agent tool providers. Any agent that supports MCP can be connected to Storybook's MCP server and use Storybook's manifests and tools. This includes popular agents like [Claude Code](https://code.claude.com/docs/en/mcp), [OpenAI Codex](https://developers.openai.com/codex/mcp/), [Cursor](https://docs.cursor.com/context/model-context-protocol), [Google Gemini CLI](https://geminicli.com/docs/tools/mcp-server/), [VS Code Copilot](https://code.visualstudio.com/docs/copilot/customization/mcp-servers), and many more.

### Which frameworks are supported?

The development and testing toolsets work in any Storybook project.
The docs toolset needs a [components manifest](https://storybook.js.org/docs/ai/manifests.md), which today is generated by the React frameworks, `@storybook/angular-vite`, and `@storybook/vue3-vite`.
See [framework support](#framework-support) for the full picture, including which frameworks produce a component API for your agent.

We are expanding manifest support to the remaining core renderers.
