...
Storybook Vite builder bundles your components and stories with [Vite](https://vitejs.dev/), a fast ESM bundler. It is the default builder for Storybook and is recommended in most cases.
...
### Set configLoader for vite config

You can pass the `configLoader` argument to `vite` by setting it in builder options. This is equivalent to the `--configLoader` command line argument.
...
Check `vite`'s [documentation](https://vite.dev/config/) for valid `configLoader` options.
...
#### Importing Markdown files as strings

Storybook's Webpack builder registers a rule that turns every `.md` import into a string, so `import readme from './readme.md'` needs no configuration.
Vite has no equivalent rule and instead tries to parse the file as JavaScript, which fails:

```text
error during build:
src/readme.md (3:26): Expected ident (Note that you need plugins to import files that are not JavaScript)
file: /my-project/src/readme.md:3:26

1: # Hello
2:
3: Some **markdown** content.
                             ^
```

Append Vite's [`?raw` suffix](https://vite.dev/guide/assets#importing-asset-as-string) to those imports to get the file contents as a string:

```diff title="src/components/Button.stories.ts"
- import readme from './readme.md';
+ import readme from './readme.md?raw';
```

Storybook's Webpack builder understands `?raw` too, so you can make this change before you migrate.
The same applies to any other file type Vite does not handle on its own.
