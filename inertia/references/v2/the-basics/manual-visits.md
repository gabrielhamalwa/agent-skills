...
In addition to [creating links](/docs/v2/the-basics/links), it's also possible to manually make Inertia visits / requests programmatically via JavaScript. This is accomplished via the `router.visit()` method.
...
For more information on uploading files, check out the dedicated [file uploads](/docs/v2/the-basics/file-uploads) documentation.
...
If you need access to the current page's props, you may pass a function to the `props` option. This function receives the current props (including any [once props](/docs/v2/data-props/once-props)) and should return the new props.
...
The function also receives [once props](/docs/v2/data-props/once-props) as a second argument. This is useful when you want to replace all regular props while still preserving once props.
...
When navigating between pages, Inertia mimics default browser behavior by automatically resetting the scroll position of the document body (as well as any [scroll regions](/docs/v2/advanced/scroll-management#scroll-regions) you've defined) back to the top of the page.
...
For more information regarding this feature, please consult the [scroll management](/docs/v2/advanced/scroll-management) documentation.
...
For more information on this feature, please consult the [partial reloads](/docs/v2/data-props/partial-reloads) documentation.

## View Transitions

You may enable [View transitions](/docs/v2/the-basics/view-transitions) for a visit by setting the `viewTransition` option to `true`. This will use the browser's View Transitions API to animate the page transition.
...
In addition to Inertia's [global events](/docs/v2/advanced/events), Inertia also provides a number of per-visit event callbacks.
