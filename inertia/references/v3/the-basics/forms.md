...
In addition to `action` and `method`, the `<Form>` component accepts several props. Many of them are identical to the options available in Inertia's [visit options](/docs/v3/the-basics/manual-visits).
...
The `<Form>` component emits all the standard visit [events](/docs/v3/advanced/events) for form submissions.
...
<Note>
  Both the `<Form>` component and `useFormContext` accept generic type parameters for type-safe errors and slot props. See the [TypeScript](/docs/v3/advanced/typescript#form-component) documentation for details.
</Note>
...
<Note>
  ...
  page](/docs/v3/core-concepts/the-protocol#request-headers) for implementation
  details.
</Note>
...
The submit methods support all of the typical [visit options](/docs/v3/the-basics/manual-visits), such as `preserveState`, `preserveScroll`, and event callbacks, which can be helpful for performing tasks on successful form submissions. For example, you might use the `onSuccess` callback to reset inputs to their original state.
...
For a more thorough discussion of form validation and errors, please consult the [validation documentation](/docs/v3/the-basics/validation).
...
You may customize the duration of the `recentlySuccessful` state by setting the `form.recentlySuccessfulDuration` option in your [application defaults](/docs/v3/installation/client-side-setup#configuring-defaults). The default value is `2000` milliseconds.
...
To instruct Inertia to store a form's data and errors in [history state](/docs/v3/data-props/remembering-state), you can provide a unique form key as the first argument when instantiating your form.
...
When using Inertia, you don't typically inspect form responses client-side like you would with traditional XHR/fetch requests. Instead, your server-side route or controller issues a [redirect](/docs/v3/the-basics/redirects) response after processing the form, often redirecting to a success page.
...
For a complete guide on validation error handling, including error bags and advanced scenarios, see the [validation documentation](/docs/v3/the-basics/validation).
...
For more information on file uploads, including progress tracking, see the [file uploads documentation](/docs/v3/the-basics/file-uploads).

## Optimistic Updates

Both the `<Form>` component and `useForm` helper support optimistic updates, allowing you to update the UI immediately before the server responds. For more details, see the [optimistic updates](/docs/v3/the-basics/optimistic-updates) documentation.

## Non-Inertia Submissions

Using Inertia to submit forms works great for the vast majority of situations. For standalone HTTP requests that don't trigger page visits, you may use the [`useHttp`](/docs/v3/the-basics/http-requests) hook, which provides the same developer experience as `useForm`. You're also free to make plain XHR or `fetch` requests using the library of your choice.
