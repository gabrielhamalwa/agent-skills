...
To create links to other pages within an Inertia app, you will typically use the Inertia `<Link>` component. This component is a light wrapper around a standard anchor `<a>` link that intercepts click events and prevents full page reloads. This is [how Inertia provides a single-page app experience](/docs/v3/core-concepts/how-it-works) once your application has been loaded.
...
<Info>
  Creating anchor (`<a>`) links that issue `POST`, `PUT`, `PATCH`, or `DELETE` requests is discouraged because it causes accessibility issues with "Open Link in New Tab / Window". The component automatically renders a `<button>` element when using these methods.
</Info>
...
For more information on managing scroll position, check out the documentation on [scroll management](/docs/v3/advanced/scroll-management).
...
For more information on this topic, check out the complete documentation on [partial reloads](/docs/v3/data-props/partial-reloads).

## View Transitions

You may enable [View transitions](/docs/v3/the-basics/view-transitions) for a link by setting the `viewTransition` prop to `true`. This will use the browser's View Transitions API to animate the page transition.
...
Using this approach, you're not limited to just setting class names. You can use this technique to conditionally render any markup on active state, such as different link text or even an SVG icon that indicates the link is active.
