...
In addition to [creating links](/docs/v3/the-basics/links), it's also possible to manually make Inertia visits or requests programmatically via JavaScript. This is accomplished via the `router.visit()` method.
...
For more information on uploading files, check out the dedicated [file uploads](/docs/v3/the-basics/file-uploads) documentation.
...
All of the parameters are optional. By default, all passed parameters (except `errorBag`) will be merged with the current page. This means you are responsible for overriding the current page's URL, component, and props.

If you need access to the current page's props, you may pass a function to the `props` option. This function receives the current props (including any [once props](/docs/v3/data-props/once-props)) and should return the new props.
...
The function also receives [once props](/docs/v3/data-props/once-props) as a second argument. This is useful when you want to replace all regular props while still preserving once props.
...
When navigating between pages, Inertia mimics default browser behavior by automatically resetting the scroll position of the document body (as well as any [scroll regions](/docs/v3/advanced/scroll-management#scroll-regions) you've defined) back to the top of the page.
...
For more information regarding this feature, please consult the [scroll management](/docs/v3/advanced/scroll-management) documentation.
...
For more information on this feature, please consult the [partial reloads](/docs/v3/data-props/partial-reloads) documentation.

## View Transitions

You may enable [View transitions](/docs/v3/the-basics/view-transitions) for a visit by setting the `viewTransition` option to `true`. This will use the browser's View Transitions API to animate the page transition.
...
The `onCancel()` and `onFinish()` event callbacks will be executed when a visit is canceled.

## Event Callbacks

In addition to Inertia's [global events](/docs/v3/advanced/events), Inertia also provides a number of per-visit event callbacks.
...
Returning `false` from the `onBefore()` callback will cause the visit to be canceled. Returning `false` from `onHttpException()` or `onNetworkError()` will prevent the corresponding [global event](/docs/v3/advanced/events) from being fired and its default behavior.
