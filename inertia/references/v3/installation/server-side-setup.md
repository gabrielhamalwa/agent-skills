...
The first step when installing Inertia is to configure your server-side framework. Inertia maintains an official server-side adapter for [Laravel](https://laravel.com/). For other frameworks, please see the [community adapters](/docs/v3/installation/community-adapters).
...
<Steps>
  ...
  <Step title="Setup root template">
    ...
    If you change the `id` of the root element, be sure to update it [client-side](/docs/v3/installation/client-side-setup#defining-a-root-element) as well.

    The `<x-inertia::head>` component also accepts [fallback content](/docs/v3/the-basics/title-and-meta#ssr-fallback) via its slot for managing head elements when [SSR](/docs/v3/advanced/server-side-rendering) is not active.
  ...
  <Step title="Register middleware">
    ...
    This middleware provides a `version()` method for setting your [asset version](/docs/v3/advanced/asset-versioning), as well as a `share()` method for defining [shared data](/docs/v3/data-props/shared-data).
  </Step>

  <Step title="Create your first response">
    That's it, you're ready to start creating Inertia [pages](/docs/v3/the-basics/pages) and rendering them via [responses](/docs/v3/the-basics/responses).
...
Prior to v3, Inertia used `@inertiaHead` and `@inertia` Blade directives to render head and body content. These directives are still supported but do not offer SSR head [fallback content](/docs/v3/the-basics/title-and-meta#ssr-fallback). The Blade components above are recommended for new applications.
