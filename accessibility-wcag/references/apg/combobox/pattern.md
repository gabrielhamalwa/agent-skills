...
A combobox is an input widget that has an associated popup. The popup enables users to choose a value for the input from a collection. The popup may be a [listbox](../listbox/pattern.md), [grid](../grid/pattern.md), [tree](../treeview/pattern.md), or [dialog.](../dialog-modal/pattern.md)
...
- [Select-Only Combobox](examples/combobox-select-only.html): A single-select combobox with no text input that is functionally similar to an HTML `select` element.
- [Editable Combobox with Both List and Inline Autocomplete](examples/combobox-autocomplete-both.html): An editable combobox that demonstrates the autocomplete behavior known as “list with inline autocomplete”.
- [Editable Combobox with List Autocomplete](examples/combobox-autocomplete-list.html): An editable combobox that demonstrates the autocomplete behavior known as “list with manual selection”.
- [Editable Combobox Without Autocomplete](examples/combobox-autocomplete-none.html): An editable combobox that demonstrates the behavior associated with `aria-autocomplete=none`.
- [Editable Combobox with Grid Popup](examples/grid-combo.html): An editable combobox that presents suggestions in a grid, enabling users to navigate descriptive information about each suggestion.
- [Date Picker Combobox](examples/combobox-datepicker.html): An editable date input combobox that opens a dialog containing a calendar grid and buttons for navigating by month and year.
...
1.  DOM Focus is maintained on the combobox and the assistive technology focus is moved within the listbox using `aria-activedescendant` as described in [Managing Focus in Composites Using aria-activedescendant](../practices/keyboard-interface.md).
...
1.  DOM Focus is maintained on the combobox and the assistive technology focus is moved within the grid using `aria-activedescendant` as described in [Managing Focus in Composites Using aria-activedescendant](../practices/keyboard-interface.md).
...
1.  DOM Focus is maintained on the combobox and the assistive technology focus is moved within the tree using `aria-activedescendant` as described in [Managing Focus in Composites Using aria-activedescendant](../practices/keyboard-interface.md).
...
- The dialog implements the keyboard interaction defined in the [modal dialog pattern.](../dialog-modal/pattern.md)
...
3.  The roles, states, and properties for popup elements are defined in their respective design patterns:
    - [Listbox Roles, States, and Properties](../listbox/pattern.md)
