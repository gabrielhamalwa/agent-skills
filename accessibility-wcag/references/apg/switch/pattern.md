...
- [Switch Example](examples/switch.html): A switch based on a `div` element that turns a notification preference on and off.
- [Switch Example Using HTML Button](examples/switch-button.html): A group of 2 switches based on HTML `button` elements that turn lights on and off.
- [Switch Example Using HTML Checkbox Input](examples/switch-checkbox.html): A group of 2 switches based on HTML `input[type="checkbox"]` elements that turn accessibility preferences on and off.
