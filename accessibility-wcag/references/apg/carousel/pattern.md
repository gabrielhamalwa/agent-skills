...
- [Auto-Rotating Image Carousel with Buttons for Slide Control:](examples/carousel-1-prev-next.html) A basic image carousel that demonstrates the accessibility features necessary for carousels that rotate automatically on page load and also enables users to choose which slide is displayed with buttons for previous and next slide.
- [Auto-Rotating Image Carousel with Tabs for Slide Control:](examples/carousel-2-tablist.html) An image carousel that demonstrates accessibility features necessary for carousels that rotate automatically on page load and also enables users to choose which slide is displayed with a set of tabs.
...
- Button elements implement the keyboard interaction defined in the [button pattern](../button/pattern.md). Note: Activating the rotation control, next slide, and previous slide do not move focus, so users may easily repetitively activate them as many times as desired.
...
- If tab elements are used for slide picker controls, they implement the keyboard interaction defined in the [Tabs Pattern.](../tabs/pattern.md)
...
- Tabbed: Has basic controls plus a single tab stop for slide picker controls implemented using the [tabs pattern.](../tabs/pattern.md)
- Grouped: Has basic controls plus a series of tab stops in a group of slide picker controls where each control implements the [button pattern.](../button/pattern.md) Because each slide selector button adds an element to the page tab sequence, this style is the least friendly for keyboard users.

### Basic carousel elements

- A carousel container element that encompasses all components of the carousel, including both carousel controls and slides, has either role region or role group. The most appropriate role for the carousel container depends on the information architecture of the page. See the [Landmark Regions Practice](../practices/landmark-regions.md) to determine whether the carousel warrants being designated as a landmark region.
...
- The rotation control, next slide control, and previous slide control are either native button elements (recommended) or implement the [button pattern](../button/pattern.md).
...
- It has slide picker controls implemented using the [tabs pattern](../tabs/pattern.md) where:
  ...
  - The `tab`, `tablist`, and `tabpanel` implement the properties specified in the [tabs pattern](../tabs/pattern.md).
