...
- [File Directory Treeview Example Using Computed Properties](examples/treeview-1a.html): A file selector tree that demonstrates browser support for automatically computing `aria-level`, `aria-posinset` and `aria-setsize` based on DOM structure.
- [File Directory Treeview Example Using Declared Properties](examples/treeview-1b.html): A file selector tree that demonstrates how to explicitly define values for `aria-level`, `aria-posinset` and `aria-setsize`.
- [Navigation Treeview Example](examples/treeview-navigation.html): A tree that provides navigation to a set of web pages and demonstrates browser support for automatically computing `aria-level`, `aria-posinset` and `aria-setsize` based on DOM structure.
...
1.  DOM focus (the active element) is functionally distinct from the selected state. For more details, see [this description of differences between focus and selection](../practices/keyboard-interface.md).
2.  The `tree` role supports the aria-activedescendant property, which provides an alternative to moving DOM focus among `treeitem` elements when implementing keyboard navigation. For details, see [Managing Focus in Composites Using aria-activedescendant](../practices/keyboard-interface.md).
