...
Like an HTML `table` element, a WAI-ARIA table is a static tabular structure containing one or more rows that each contain one or more cells; it is not an interactive widget. Thus, its cells are not focusable or selectable. The [grid pattern](../grid/pattern.md) is used to make an interactive widget that has a tabular structure.
...
- If the table contains sortable columns or rows, aria-sort is set to an appropriate value on the header cell element for the sorted column or row as described in the [Grid and Table Properties Practice](../practices/grid-and-table-properties.md).
- If there are conditions where some rows or columns are hidden or not present in the DOM, e.g., there are widgets on the page for hiding rows or columns, the following properties are applied as described in the [Grid and Table Properties Practice](../practices/grid-and-table-properties.md).
...
- If the table includes cells that span multiple rows or multiple columns, then aria-rowspan or aria-colspan is applied as described in the [Grid and Table Properties Practice](../practices/grid-and-table-properties.md).
